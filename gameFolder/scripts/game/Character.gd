...
func _ready() -> void:
	...
	if not is_player and flip_if_opponent:
		...
		if miss_anims.size() > 0:
			for _i in miss_anims.size():
				var i:String = miss_anims[_i]
				if i == "singLEFTmiss": miss_anims[_i] = "singRIGHTmiss"
				if i == "singRIGHTmiss": miss_anims[_i] = "singLEFTmiss"
